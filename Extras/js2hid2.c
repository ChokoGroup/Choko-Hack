		...
		if (read(joy_fd0, &event, sizeof(struct input_event)) > 0) {
			switch (event.type) {
				...
				case 1:
					...
					value   0 or 1
					*/
					if (event.value == 1) buttons0 = buttons0 | (1 << (event.code % 8));
					else buttons0 = buttons0 & ~(1 << (event.code % 8));
					break;
			}
			if (axis0[0] == 0) fputc(0x81, hid_fd0);				/* X = left */
			else if (axis0[0] == 255) fputc(0x7F, hid_fd0);			/* X = right */
			else fputc(0x00, hid_fd0);								/* X = center */
			if (axis0[1] == 0) fputc(0x81, hid_fd0);				/* Y = up */
			else if (axis0[1] == 255) fputc(0x7F, hid_fd0);			/* Y = down */
			else fputc(0x00, hid_fd0);								/* Y = center */
		...
		if (read(joy_fd1, &event, sizeof(struct input_event)) > 0) {
			switch (event.type) {
				case 3:
					...
					break;
				case 1:
					if (event.value == 1) buttons1 = buttons1 | (1 << (event.code % 8));
					else buttons1 = buttons1 & ~(1 << (event.code % 8));
					break;
			}
			if (axis1[0] == 0) fputc(0x81, hid_fd1);			/* X = left */
			else if (axis1[0] == 255) fputc(0x7F, hid_fd1);		/* X = right */
			else fputc(0x00, hid_fd1);							/* X = center */
			if (axis1[1] == 0) fputc(0x81, hid_fd1);			/* Y = up */
			else if (axis1[1] == 255) fputc(0x7F, hid_fd1);		/* Y = down */
			else fputc(0x00, hid_fd1);							/* Y = center */
